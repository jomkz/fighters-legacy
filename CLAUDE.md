...
- `engine/net/WorldBroadcaster.h` — `ISimUpdate` + `INetworkEventHandler`; constructor takes optional `WeatherController* weather = nullptr`; spawns/kills peer entities and their `FlightIntegrator` instances on connect/disconnect; decodes `MsgClientInput` → `PeerInputState`; steps each peer's `FlightIntegrator` with `WindInfluence` from `WeatherController` each tick, calls `EntityManager::onTick`, broadcasts `MsgWorldSnapshot`; broadcasts `MsgWeatherState` every 10 ticks (~6 Hz); `m_peerEntities` + `m_peerInputs` + `m_peerFlightSims` maps (keyed by peerId); `getPeerCount() const noexcept` reads `std::atomic<int> m_activePeerCount` — safe cross-thread read for the LAN beacon
