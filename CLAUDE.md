...
- **`engine/net/WorldBroadcaster`**: `ISimUpdate` + `INetworkEventHandler`. `onConnect` checks `m_bannedAddresses` first (calls `INetwork::disconnectPeer` and returns early if banned), then sends `MsgHello` (reliable), spawns `"builtin:debug-entity"` for the peer, creates a `FlightIntegrator` (via `BuiltinFlightModel::get()`) stored in `m_peerFlightSims`, stores the entity in `m_peerEntities`, and sends `MsgConnectAck` + `MsgEntityTypeDef[]` (reliable); `onDisconnect` kills the entity and erases both maps. `onReceive` validates `MsgClientInput::protocolVersion == kProtocolVersion` (discards + warns on mismatch) then stores → `PeerInputState` (`m_peerInputs`). `onTick` steps each peer's `FlightIntegrator` from stored inputs (with `WindInfluence` derived from `WeatherController`), copies `FlightState` → `EntityState`, then calls `EntityManager::onTick` and serialises all live entities into `MsgWorldSnapshot` (header carries `protocolVersion`) and broadcasts unreliably; also ticks `WeatherController` and broadcasts `MsgWeatherState` every 10 ticks (~6 Hz). Constructor takes optional `WeatherController* weather = nullptr`. `engine-net` privately links `engine-flight` and `engine-weather`. `getPeerCount() const noexcept` — reads `std::atomic<int> m_activePeerCount`; safe to call from any thread. **Peer management (sim-thread only):** `kickPeer(peerId)` — calls `INetwork::disconnectPeer`; `banAddress(ip)` — normalises the IP (strips brackets + `::ffff:` prefix), inserts into `m_bannedAddresses`, kicks currently-connected matching peers; `unbanAddress(ip)` — erases from ban set; `forEachPeer(fn)` — iterates `m_peerEntities`, copies each `getPeerAddress` result into a `std::string` before calling fn (avoids stale-pointer UB from the single `m_peerAddressBuf` in `ENetNetwork`).
...
- `engine/net/WorldBroadcaster.h` — `ISimUpdate` + `INetworkEventHandler`; constructor takes optional `WeatherController* weather = nullptr`; spawns/kills peer entities and their `FlightIntegrator` instances on connect/disconnect; ban check in `onConnect` (`m_bannedAddresses` set); `getPeerCount() const noexcept` atomic cross-thread read; **sim-thread peer management:** `kickPeer(peerId)`, `banAddress(ip)` (normalises + kicks matches), `unbanAddress(ip)`, `forEachPeer(fn)` (copies addr string per entry); `m_bannedAddresses` in-memory ban set; `m_peerEntities` + `m_peerInputs` + `m_peerFlightSims` maps
...
- `server/fl-server/AdminConsole.h` — `ServerCommandContext` struct (pointers to broadcaster, entityManager, typeRegistry, weatherController, beacon, gameLoop, logger, configPath, startTime, quitFlag) + `registerServerCommands(registry, ctx)` registers 12 commands: `help`, `status` (synchronous atomic reads), `peers`, `kick <peerId|IP>`, `ban <peerId|IP>`, `unban <IP>`, `set_weather`, `set_time`, `spawn`, `kill`, `reload_config` (re-reads server.toml, applies name to beacon), `quit`. Mutating commands enqueue via `GameLoop::enqueueSimCallback`. Async output printed via `std::printf` from within the callback (POSIX thread-safe). `kick`/`ban` dispatch: all-digit arg → peerId path; otherwise → IP path (normalised via `normalizeIp`). Linked by `fl-server` only; `test_admin_console` compiles `AdminConsole.cpp` directly. `platform/INetwork.h` now has `virtual void disconnectPeer(uint32_t peerId) = 0` — any new `INetwork` mock must add this stub.
