...
- `engine/net/GameProtocol.h` — five packed wire types; `MsgConnectAck` 12 bytes (+assignedEntityIdx/Gen), `MsgEntityEntry` 68 bytes (double pos), `MsgClientInput` 44 bytes; `MsgId::ClientInput = 0x03`; always parse with `memcpy`; all documented field offsets protected by `offsetof` static_asserts (requires `<cstddef>`); see `docs/network-protocol.md`
- `docs/network-protocol.md` — stable public wire protocol spec: channel assignments, struct field tables (Offset/Size/Field/Type/Notes), little-endian declaration, ARM64 alignment warnings for misaligned fields (`pos[3]` at offset 12, `tickIndex` at offset 4 in snapshot header), version negotiation placeholder (#92), bandwidth/MTU scalability guidance (single-fragment threshold ≈20 entities; interest management required for 128+ players)
