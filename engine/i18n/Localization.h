...
class Localization {
  public:
    ...
    // Each tier loaded least-specific-first; within each tier mods loaded ascending priority.
    // Reads meta.toml from the most-specific tier available to set isRTL().
    // |mods| sorted highest-priority-first (ModLoader order). Stores lang+mods for reload().
    // Returns true if at least one .toml file was loaded.
    bool load(const char* lang, const std::vector<const IContentPack*>& mods);
    ...
    std::vector<LocaleInfo> listLocales(const std::vector<const IContentPack*>& mods) const;
    ...
    std::vector<std::string> listMissingKeys(const char* lang, const std::vector<const IContentPack*>& mods) const;

    // Returns fraction of "en" keys present in |lang| (0.0-1.0). Returns 1.0 if lang=="en"
    // or if there are no "en" keys.
    float getCoverage(const char* lang, const std::vector<const IContentPack*>& mods) const;
  ...
  private:
    ...
    void loadOneLocale(const char* tag, const std::vector<const IContentPack*>& mods, StringTable& out) const;
    ...
    std::vector<const IContentPack*> m_lastMods;
