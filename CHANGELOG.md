...
- **engine**: introduce a shared `fl::IClock` clock seam (`platform/IClock.h`) with `SystemClock` (the default) and `ManualClock` (test double), replacing the per-class `std::function<…steady_clock::time_point()>` + `setClockOverride` injectable-clock pattern across `AuthTracker`, `WorldBroadcaster`, `LoadingScreen`, `ServerNotice`, `FlightInputCollector`, and `ENetNetwork`. Components now hold a `const fl::IClock*` defaulting to `SystemClock::instance()` and expose `setClock()` (`ENetNetwork`: `setPreHandshakeClock()`); tests inject a `ManualClock` and drive time via `advance()`/`set()`. No behavior change
...
- **engine**: spherical-Earth world model — `CentralGravityField` (`engine/flight/CentralGravityField.h`) implements `IGravityField` with 1/r² falloff toward a planet centre at `{0, -R, 0}`; `Geodetic.h` provides spherical lat/lon/alt ↔ world-XYZ utilities (`worldToGeodetic`, `geodeticToWorld`, `geodeticAltitude`); `IGravityField::geodeticAltitude()` virtual hook lets `FlightIntegrator` look up correct ISA pressure altitude far from the world origin; `TerrainStreamer::setSphericalPlanetRadius()` applies per-chunk Y offsets for both physics (`heightAt`) and rendering (`getRenderItems`); `WorldBroadcaster::setGravityField()` configures gravity for all integrators and transmits the planet radius to clients in `MsgConnectAck`; enabled in fl-server via `[world] spherical_earth = true` (radius configurable via `planet_radius_m`; Earth default 6 371 000 m); closes #357
- **network**: `MsgConnectAck` extended 12 → 16 bytes — new `float planetRadiusKm` field at offset 12 (0.0 = flat Earth); `ClientNetEventHandler` reads it and exposes `planetRadiusKm()` accessor; game wires it to `TerrainStreamer::setSphericalPlanetRadius()` on `Screen::Flight` entry

- **engine**: four vehicle/control seams installed in the sim path (behavior-preserving — all flight tests pass unchanged): (1) `fl::IEntityController` (`engine/entity/IEntityController.h`) decouples entity control from network peers — `WorldBroadcaster` now keys its flight registry by `EntityId` instead of peer id and steps every controlled entity uniformly; a connected player is a `PeerController`, and `registerController()` lets a server-side AI/scripted controller drive a non-peer entity that serializes into `MsgWorldSnapshot` for free; (2) `EntityDef::flightModelId` + a `WorldBroadcaster` flight-model resolver (`std::function`, wired in fl-server to `AssetManager` + `parseFlightModel`) bind a per-entity flight model at spawn, falling back to the builtin model when empty/unknown; (3) `fl::IForceModel` + `FixedWingForceModel` (`engine/flight/`) extract the fixed-wing aero/thrust math behind a swappable interface — a future rotor/multirotor/ballistic model plugs in via `FlightIntegrator::setForceModel()` without touching the F=ma core; (4) `fl::IGravityField` + `FlatGravityField` extract the gravity constant — a central/spherical field plugs in via `setGravityField()` for planet-scale trajectories. `test_force_model` directly unit-tests the force/gravity models and the integrator's dispatch to injected seams; `test_world_broadcaster` covers the non-peer-controller and flight-model-resolver paths
